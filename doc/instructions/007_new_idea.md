...
Then:
1) see if any goals or declarative cards are due (*not* unseen; cards that have been practiced before and are due again now (for declarative that means recall % below 0.9)). If more than 15 cards are due, randomly select 15. These cards should be run through with the already implemented Spaced Repetition flow ()
