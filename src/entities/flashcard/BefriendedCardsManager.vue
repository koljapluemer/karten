...
const linkedFlashcards = computed(() =>
  props.modelValue
...
const searchResults = computed(() => {
  ...
  const linked = new Set(props.modelValue)
...
const handleDetach = (id: string) => {
  emit('update:modelValue', props.modelValue.filter((fid) => fid !== id))
  showToast('Card unlinked', 'info')
}
...
<template>
  <div class="border-t pt-4">
    ...
    <p class="text-light mb-4">
      Cards to surface soon after practicing this one
    </p>

    <div
      v-if="linkedFlashcards.length > 0"
      class="mb-4"
    >
      <div class="overflow-x-auto">
        <table class="table table-sm">
          ...
          <tbody>
            <tr
              v-for="card in linkedFlashcards"
              :key="card.id"
            >
    ...
    <div class="flex flex-col gap-2">
      <fieldset class="fieldset">
        <label
          for="search-befriended"
          class="label"
        >Search flashcards</label>
        <input
          id="search-befriended"
          v-model="searchQuery"
          type="text"
          class="input"
